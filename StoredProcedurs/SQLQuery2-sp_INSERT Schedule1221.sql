...
BEGIN
	...
	DECLARE @lesson_number		AS	TINYINT	=	(SELECT COUNT(lesson_id) FROM Schedule WHERE discipline=@discipline AND [group]=@group);   -- нужно учитывать проведенные занятия по выставляемой дисицплине
	PRINT @lesson_number;
