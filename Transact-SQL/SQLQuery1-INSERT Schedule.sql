...
--В цикле перебираем занятие по номеру, определяем дату и время каждого занятияs 
DECLARE @date		    AS DATE     =  @start_date;  -- чтобы считать даты
DECLARE @lesson_number  AS TINYINT  =  1;            -- чтобы считать уроки 
DECLARE @time	AS TIME		=	@start_time;         -- от ствртовой остается неизменным 
